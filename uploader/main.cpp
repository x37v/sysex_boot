...
//we can put 28 bytes into 32 midibyte packed bytes
...
void printMidiDevices(bool input){
	for (int i = 0; i < Pm_CountDevices(); i++) {
        ...
        if (info->input && input)
			  cout << i << ", " << info->interf << ", " << info->name << endl;
...
int getPageSize(PmStream *midi_in, PmStream *midi_out, long timeout){
	...
	if(error) {
		cerr << "error writing GET PAGE SIZE msg: " << Pm_GetErrorText(error) << endl;
		exit(-1);
	}
	...
	while(startTime + timeout > Pt_Time()){
		...
		if(count != 0){
			for(unsigned int i = 0; i < 4; i++){
				unsigned char inByte = (msg.message >> (i * 8)) & 0xFF;
				if(inByte == MIDI_SYSEX_START){
					...
					if(inByte == MIDI_SYSEX_STOP){
						return pageSize;
					} else if((sysexByteCount < MIDIBOOT_SYSEX_ID_LEN)){
						//make sure that our id matches
						if(inByte == midiboot_sysex_id[sysexByteCount])
							sysexByteCount++;
						else
							inSysexMessage = false;
					} else if(sysexByteCount == MIDIBOOT_SYSEX_ID_LEN) {
						if(inByte != MIDIBOOT_GETPAGESIZE)
							inSysexMessage = false;
						else
							sysexByteCount++;
					} else if (sysexByteCount < (MIDIBOOT_SYSEX_ID_LEN + 4)){
						unsigned int index = (sysexByteCount - MIDIBOOT_SYSEX_ID_LEN - 1);
						//unpack!
						switch(index){
							case 0:
								pageSize |= ((int)inByte) << 9;
								break;
							case 1:
								pageSize |= (int)inByte << 2;
								break;
							case 2:
								pageSize |= (int)inByte >> 5;
								break;
						}
						sysexByteCount++;
					} else
		...
		if(count != 0){
			for(unsigned int i = 0; i < 4; i++){
				...
				if(inByte == MIDI_SYSEX_START){
					...
					if(sysexByteCount == MIDIBOOT_SYSEX_ID_LEN){
						if(inByte == MIDI_SYSEX_STOP){
							return true;
						} else
							inSysexMessage = false;
					} else if((sysexByteCount < MIDIBOOT_SYSEX_ID_LEN) && inByte == midiboot_sysex_id[sysexByteCount])
						sysexByteCount++;
					else 
						inSysexMessage = false;
				}
...
int main(int argc, char *argv[]){
	...
	if(pageSize == 0){
		cerr << "*** failed to get the page size from the microcontroller" << endl;
		cerr << "*** are you in boot loader mode?" << endl;
		cerr << "*** do you have both the midi input and output to/from your microcontroller connected to your computer" << endl;
		cerr << "*** did you select the correct input and output?" << endl;
		cerr << "*** aborting" << endl;
		exit(-1);
	}
	...
	for(unsigned int i = 0; i < numPages; i++){
		...
		for(unsigned int j = 0; j < pages[i]->size(); j += CHUNK_SIZE){
			...
			std::vector<unsigned char> * newMsg = new std::vector<unsigned char>;
			...
			packedPage.clear();
			midiBytePack(tmp,packedPage);

			//write the packed address+data to the sysexMessage
			for(std::vector<unsigned char>::iterator it = packedPage.begin();
					it != packedPage.end(); it++)
				newMsg->push_back(*it);
	...
	cout << "using this many sysex messages: " << sysexMessages.size() << endl;

	//upload
	cout << "uploading now" << endl;

	//send each sysex message
	for(std::vector<std::vector<unsigned char> *>::iterator it = sysexMessages.begin(); 
			it != sysexMessages.end(); it++){
		PmError error;
		...
		if(!waitForAck(midi_in, TIMEOUT_MS)){
			cerr << "FAILED: sysex message failed to acknowledge within the timeout" << endl;
			cerr << "was the microcontroller in boot loader mode?" << endl;
			exit(-1);
		} 
	}
	...
	if(error) {
		cerr << "error writing LEAVE BOOT msg: " << Pm_GetErrorText(error) << endl;
		exit(-1);
	}
